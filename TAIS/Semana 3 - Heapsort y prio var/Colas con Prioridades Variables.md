* `IndexPQ<T>` cuenta con las siguientes operaciones:
	...
	* consultar el elemento mas prioritario, `Par const& top() const`
	* eliminar el primer elemento, `void pop()`
	* determinar si la cola de prioridad es vacía, `bool empty() const`
	* consultar el numero de elementos de la cola, `int size() const`

![[Representacion colas.png]]
##### Implementación
