...
```cpp
Set //conjunto vacio
bool insert(T const& e) //insertar un elemento - Coste O(log N)
bool erase(T const& e) //eliminar un elemento - Coste O(log N)
bool contains(T const& e) const //pertenencia al conjunto - Coste O(log N)
bool empty() const //el conjunto es vacio - Coste O(1)
int size() const //el cardinal del conjunto - Coste O(1)
```

* Rotaciones - Coste O(1)
## Implementación
![[Implementacion AVL.png]]
```cpp
//
//  TreeSet_AVL.h
//
